...
def _make_desc_df_raw(df_: pd.DataFrame, tasks: List[str]) -> pd.DataFrame:
    """
    Descriptor subset을 raw 상태로 생성.
    여기서는 NaN/Inf imputation, scaling을 수행하지 않음.
    """
...
def build_desc_df_scaled(
    ...
    Train:
        ...
        -> columns 생성, median 계산, StandardScaler fit

    Val/Test/External:
        fixed_cols=train_cols, scaler=train_scaler, impute_values=train_impute
        -> train schema로 reindex, train median으로 impute, train scaler로 transform
    """
...
# NOTE:
# 기존 model.py의 DEFAULT_NODE_IN_DIM=89와 호환성을 유지하기 위해
# 기존 atom list 차원을 유지함. 여기서 원소 리스트를 줄이면 model.py도 같이 수정해야 함.
ATOM_LIST = [
...
def _make_fold_cache_path(
    ...
    """
    fold별 graph/data_list cache 경로 생성.
    dataset file size/mtime까지 반영해서 CSV가 바뀌면 자동으로 다른 cache 사용.
    """
...
def build_scaffold_kfold_loader(
    ...
    첫 실행:
        raw CSV → scaffold split → fold별 descriptor scaler fit/transform
        → graph conversion → fold cache 저장

    이후 실행:
        fold cache load → DataLoader 생성
    ...
    for fold_idx in range(n_splits):
        ...
        # fold train에서만 descriptor scaler fit
        ...
        # fold val은 train scaler로 transform
...
def build_external_loader(
    ...
    반드시 train descriptor schema/scaler/impute_values를 넣어야 함.
    """
